...
mock_optimal_max_mark_string = Model.array([0, 1, 1, 2])
mock_not_optimal_max_mark_string = Model.array([0, -1, 1, -2])

mock_optimal_min_mark_string = Model.array([0, -1, -1, -2])
mock_not_optimal_min_mark_string = Model.array([0, 1, -1, 2])

mock_not_optimal_columns = Model.array([[0, -1, -1, 1], [0, 1, -1, 2], [-1, -1, -1, -2], [0, 3, 2, -1]]).T
mock_not_optimal_columns_with_one_available_column = Model.array(
...
mock_solved_f = Fraction('8/3')

mock_b = Model.array([5, 4, 3, 2])
...
class SimplexMethodTestCase(unittest.TestCase):
    ...
    def test__recalculate_with_new_basis(self):
        ...
        np.testing.assert_array_equal(mock_solved_b, method._model.b)
        np.testing.assert_array_equal(mock_solved_marks, method._model.c)
        np.testing.assert_array_equal(mock_solved_columns, method._model.a)
        np.testing.assert_equal(mock_solved_f, method._model.f)
